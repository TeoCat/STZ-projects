...
void RungeSolver::Solver_set_matrix() {
    _solver._b.clear();
...
void RungeSolver::Calculate() {
    ...
    std::cout << "max_diff (Runge) : " << max_diff << std::endl;
    std::cout << "step (Runge) : " << Get_step() << std::endl;

}

void DPSolver::calc_step() {
    Eigen::VectorXd x1;
    Eigen::VectorXd x2;
    Eigen::MatrixXd tmp;
    double diff;
...
void DPSolver::Solver_set_matrix() {
    _solver._b.clear();
...
void DPSolver::Calculate() {
    ...
    std::cout << "min_step (DP) : " << min_step << std::endl;
    std::cout << "total_steps (DP) : " << total_steps << std::endl;
}

Eigen::MatrixXd Solver::Generate_mat() {
...
void EquationSolver::Set_solver(std::unique_ptr<Solver> &&solver) {
    _solver = std::move(solver);
    _solver->Solver_set_matrix();
}

void EquationSolver::Reload_values(const Eigen::VectorXd &init_vals, double init_time) {
...
void Calculate(std::function<Eigen::VectorXd(double, const Eigen::VectorXd &)> src_function,
    ...
    std::unique_ptr<Solver> runge_solver = std::make_unique<RungeSolver>(eq_solver);
    eq_solver.Set_solver(std::move(runge_solver));
    eq_solver.Solve();

    std::unique_ptr<Solver> dp_solver = std::make_unique<DPSolver>(eq_solver);
